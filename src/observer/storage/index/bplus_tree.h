...
public:
  ...
  int operator()(const char *v1, const char *v2) const {
    ...
    for(int i=0; i<attr_types_.size(); ++i){
      ...
      case INTS: 
      case DATES: {
        int ret = compare_int((void *)(v1+start_idx), (void *)(v2+start_idx), attr_lengths_[i]);
        if(ret != 0) return ret;
        break;
      }
      case FLOATS: {
        int ret = compare_float((void *)(v1+start_idx), (void *)(v2+start_idx), attr_lengths_[i]);
        if(ret != 0) return ret;
        break;
      }
      case CHARS: {
        int ret = compare_string((void *)(v1+start_idx), attr_lengths_[i], (void *)(v2+start_idx), attr_lengths_[i]);
        if(ret != 0) return ret;
        break;
      }
      default:{
...
public:
  ...
  std::string operator()(const char *v) const {
    ...
    for(int i=0; i<attr_types_.size(); ++i){
      ...
      switch (attr_types_[i]) {
      case INTS: {
        char* tmp = new char[attr_lengths_[i]+1];
        memset(tmp, 0, sizeof(char)*(attr_lengths_[i]+1));
        memcpy(tmp, v+start_idx, attr_lengths_[i]);
        int date = *(int *)tmp;
        delete tmp;
        str.append(std::to_string(date));
        break;
      }
      case DATES: {
        char* tmp = new char[attr_lengths_[i]+1];
        ...
        int date = *(int *)tmp;
        delete tmp;
        str.append(std::to_string(date/10000));
        str.push_back('-');
        str.append(std::to_string((date%10000)/100));
        str.push_back('-');
        str.append(std::to_string(date%100));
        break;
      }
      case FLOATS: {
        char* tmp = new char[attr_lengths_[i]+1];
      ...
      case CHARS: {
        for (int i = 0; i < attr_lengths_[i]; ++i) {
          if (attr_types_[i+start_idx] == 0) {
            break;
          }
          str.push_back(attr_types_[i+start_idx]);
        }
        return str;
      }
      default:{
...
struct IndexFileHeader {
  ...
  int32_t  attr_num;
  int32_t  attr_length;
  AttrType attr_types[MAX_ATTR_NUM];
  int32_t  attr_lengths[MAX_ATTR_NUM];
...
public:
  ...
  /**
   * 此函数创建一个名为fileName的multi-index。
   * field_metas中保存了索引每个属性对应的FieldMeta，从中可以获取属性的类型和长度
   */
  RC create(const char *file_name, const std::vector<FieldMeta> &field_metas,
	    int internal_max_size = -1, int leaf_max_size = -1);
...
private:
  ...
  char *make_key(const char *user_key, std::vector<FieldMeta> &field_metas, const RID &rid);
