...
char *BplusTreeHandler::make_key(const char *user_key, std::vector<FieldMeta> &field_metas, const RID &rid)
{
  char *key = (char *)mem_pool_item_->alloc();
  if (key == nullptr) {
    LOG_WARN("Failed to alloc memory for key.");
  ...
  int key_start = 0;
  for(int i=0; i<field_metas.size(); ++i){
    memcpy(key+key_start, user_key+field_metas[i].offset(), field_metas[i].len());  // 这里认为field_metas[i].len()的值和file_header_中存的attr_lengths[i]相等，不太严谨
    key_start += field_metas[i].len();
  }
  memcpy(key + key_start, &rid, sizeof(rid));
  return key;
}
...
RC BplusTreeHandler::insert_entry(const char *user_key, std::vector<FieldMeta> &field_metas, const RID *rid)
{
  ...
  char *key = make_key(user_key, field_metas, *rid);
  if (key == nullptr) {
    LOG_WARN("Failed to alloc memory for key.");
    return RC::NOMEM;
  }
