    ...
    for (auto it = mtr_manager->log_redo_list.begin(); it != mtr_manager->log_redo_list.end(); it++) {
      ...
      switch(clog_record->get_log_type()) {
        ...
        case CLogType::REDO_UPDATE: {
          char *record_data = new char[clog_record->log_record_.upd.new_data_len_];
          memcpy(record_data, clog_record->log_record_.upd.new_data_, clog_record->log_record_.upd.new_data_len_);
          Record record;
          record.set_data(record_data);
