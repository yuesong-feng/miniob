  ...
  switch (flag) {
    ...
    case REDO_DELETE: {
      if (!rec) {
        ...
        strcpy(log_record_.del.table_name_, table_name);
...
CLogRecord::CLogRecord(char *data)
{
  ...
  switch (flag_) {
    ...
    case REDO_UPDATE: {
      ...
      data += sizeof(RID);
      log_record_.upd.new_data_len_ = *(int *)data;
      data += sizeof(int);
      log_record_.upd.new_data_ = new char[log_record_.upd.hdr_.logrec_len_ - CLOG_UPD_REC_NODATA_SIZE];
      memcpy(log_record_.upd.new_data_, data, log_record_.upd.new_data_len_);
    } break;
    default:
      LOG_ERROR("flag is error");
      break;
...
CLogRecord::~CLogRecord()
{
  ...
  if (REDO_UPDATE == flag_) {
    delete[] log_record_.upd.new_data_;
    // delete[] log_record_.upd.old_data_;
  }
}
...
RC CLogRecord::copy_record(void *dest, int start_off, int copy_len)
{
  ...
  if (start_off + copy_len > get_logrec_len()) {
    ...
    if (start_off > CLOG_UPD_REC_NODATA_SIZE) {
      memcpy(dest, log_rec->upd.new_data_ + start_off - CLOG_UPD_REC_NODATA_SIZE, copy_len);
